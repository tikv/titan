...
TEST(UtilTest, Compression) {
  ...
  for (auto compression :
    ...
    CompressionInfo compression_info(compression_opt, compression_ctx,
    ...
    if (compression != kNoCompression) {
      ...
      UncompressionInfo uncompression_info(uncompression_ctx, UncompressionDict::GetEmptyDict(), compression);
      OwnedSlice output;
