...
class BlobFileTest : public testing::Test {
 public:
  ...
  void TestBlobFileReader(TitanOptions options) {
    ...
    ReadOptions ro;
    std::unique_ptr<RandomAccessFileReader> random_rw_file_reader;
    ASSERT_OK(NewBlobFileReader(file_number_, 0, db_options, env_options_, env_,
                                &random_rw_file_reader));
    std::unique_ptr<BlobFileReader> blob_file_reader;
    ASSERT_OK(BlobFileReader::Open(cf_options, std::move(random_rw_file_reader),
