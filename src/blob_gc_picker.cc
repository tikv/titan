...
std::unique_ptr<BlobGC> BasicBlobGCPicker::PickBlobGC(
  ...
  for (auto& gc_score : blob_storage->gc_score()) {
    ...
    if (!stop_picking) {
        ...
        if (batch_size >= cf_options_.max_gc_batch_size && estimate_output_size >= min_output_threshold) {
            // stop pick file for this gc, but still check file for whether need trigger gc after this
            stop_picking = true;
        }
    }else{
        ...
        if (next_gc_size > cf_options_.min_gc_batch_size){
            ...
            ROCKS_LOG_INFO(db_options_.info_log, "remain %zu bytes to be gc and trigger after this gc",
                           next_gc_size);
            break;
        }
    }
  ...
  ROCKS_LOG_INFO(db_options_.info_log, "got batch size %zu, min output threshold:%zu, estimate output %zu bytes",
  ...
  return std::unique_ptr<BlobGC>(
      new BlobGC(std::move(blob_files), std::move(cf_options_), maybe_continue_next_time));
}
